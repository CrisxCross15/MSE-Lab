...
static void lcd_sNibble_E(uint8_t nibble, uint8_t rs)
{
    ...
    /* Set data pins D4–D7 according to the upper 4 bits of nibble */
...
LCD_Status_t lcd_print_string(const char *str)   // cambiar void → LCD_Status_t
{
    while (*str != '\0')
    {
        lcd_print_char(*str);
